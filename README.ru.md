...
Админка и виджет говорят по-русски и по-английски. Язык интерфейса каждый менеджер выбирает себе — в одной команде бывает и русский, и англоговорящий, и общая настройка заставила бы одного из них терпеть.

Язык бота задаётся отдельно, на вкладке «Как отвечает». Он определяет подписи в виджете и правила, которые читает модель: русские правила тянут русский ответ даже на английском сайте, и это не лечится инструкциями.

Добавить язык — перевести один файл:
...
Ключ перевода — сама русская строка, как в gettext. Поэтому в шаблоне видно, что выведется, без похода в словарь, а непереведённая строка остаётся русской, а не превращается в `nav.bots`. Цена одна: правка русской формулировки рвёт связь с переводом, и её ловит `go test ./internal/lang` — он сверяет словари с кодом и показывает как пропуски, так и осиротевшие строки.
...
```bash
docker compose up -d
docker compose run --rm hark -manager you@example.com -password секрет
```

Образ 42 МБ, процесс работает не от рута, база — в именованном томе. SQLite здесь на чистом Go, поэтому cgo не нужен, бинарник статический и без боли собирается под arm64 для Raspberry Pi.
